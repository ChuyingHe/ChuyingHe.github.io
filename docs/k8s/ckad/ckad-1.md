...
>  -- -- 
> **方法二**：
> （1）更新ReplicaSet，删除旧的Pod: `kubectl edit replicaset xxx` 
...
> 每个Namespace都有自己的参数设置和限制，能保证不占用过多的资源（比如说储存空间等）。Namespace常见的应用场景是：一个`dev`给开发，一个`prod`的Namespace在使用中。
...
> ⚠️ 命名格式是`[ServiceName].[Namespace].svc.cluster.local`。
> -- -- 
